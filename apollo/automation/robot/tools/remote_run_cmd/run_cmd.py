...
class RunCmd():
    def __init__(self, cmd, std_tmp_path = None, timeout = None):
        ...
        self.pid = os.getpid()
        self.stdout_file = std_tmp_path + "/stdout_tmp_%s_%s" % (self.pid, str(int(time.time())))
        self.stderr_file = std_tmp_path + "/stderr_tmp_%s_%s" % (self.pid, str(int(time.time())))
    ...
    def run(self):
        ...
        self.process = subprocess.Popen(self.cmd, shell=True, stderr = self.fp_stderr, stdout = self.fp_stdout)
        while True:
